...
public:
	...
	/**
	 * Attempts to activate any ability in the specified item slot. Will return false if no activatable ability found or
	 * activation fails Returns true if it thinks it activated, but it may return false positives due to failure later
	 * in activation. If bAllowRemoteActivation is true, it will remotely activate local/server abilities, if false it
	 * will only try to locally activate the ability
	 */
	 ...
	 * If bAllowRemoteActivation is true, it will remotely activate local/server abilities, if false it will only try to
	 * locally activate the ability
	 */
	UFUNCTION(BlueprintCallable, Category = "Abilities")
	bool ActivateAbilitiesWithTags(FGameplayTagContainer AbilityTags, bool bAllowRemoteActivation = true);

	/** Returns a list of active abilities matching the specified tags. This only returns if the ability is currently
	 * running */
	UFUNCTION(BlueprintCallable, Category = "Abilities")
...
protected:
	...
	/** Abilities to grant to this character on creation. These will be activated by tag or event and are not bound to
	 * specific inputs */
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = Inventory)
	TMap<FRPGItemSlot, TSubclassOf<UGameplayAbility>> SlottedAbilitiesByClass;
	...
	void FillSlottedAbilitySpecs(TMap<FRPGItemSlot, FGameplayAbilitySpec>& SlottedAbilitySpecs,
								 TMap<FRPGItemSlot, TSubclassOf<UGameplayAbility>>& SlottedAbilityByClass);
	...
	virtual void HandleDamage(float DamageAmount, const FHitResult& HitInfo,
							  const struct FGameplayTagContainer& DamageTags, ARPGCharacterBase* InstigatorCharacter,
							  AActor* DamageCauser);
