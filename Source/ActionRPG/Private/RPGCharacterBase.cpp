...
void ARPGCharacterBase::FillSlottedAbilitySpecs(
	...
	TMap<FRPGItemSlot, TSubclassOf<UGameplayAbility>>& SlottedAbilityByClass)
{
	...
	for (const TPair<FRPGItemSlot, TSubclassOf<URPGGameplayAbility>>& DefaultPair : DefaultSlottedAbilities)
	{
		...
		SlottedAbilityByClass.Add(DefaultPair.Key, DefaultPair.Value);
	}
		...
		for (const TPair<FRPGItemSlot, URPGItem*>& ItemPair : SlottedItemMap)
		{
			...
			if (SlottedItem && SlottedItem->GrantedAbility)
			{
				...
				SlottedAbilitySpecs.Add(ItemPair.Key, FGameplayAbilitySpec(SlottedItem->GrantedAbility, AbilityLevel,
																		   INDEX_NONE, SlottedItem));

				SlottedAbilityByClass.Add(ItemPair.Key, SlottedItem->GrantedAbility);
			}
		}
	}
}
...
{
	TMap<FRPGItemSlot, FGameplayAbilitySpec> SlottedAbilitySpecs;
	FillSlottedAbilitySpecs(SlottedAbilitySpecs, SlottedAbilitiesByClass);
	...
	for (const TPair<FRPGItemSlot, FGameplayAbilitySpec>& SpecPair : SlottedAbilitySpecs)
	{
		...
		if (GetLocalRole() == ROLE_Authority && !SpecHandle.IsValid())
		{
...
void ARPGCharacterBase::PossessedBy(AController* NewController)
{
	...
	if (InventorySource)
	{
		InventoryUpdateHandle =
			InventorySource->GetSlottedItemChangedDelegate().AddUObject(this, &ARPGCharacterBase::OnItemSlotChanged);
...
{
	TSubclassOf<UGameplayAbility>* FoundAbility = SlottedAbilitiesByClass.Find(ItemSlot);

	if (FoundAbility && AbilitySystemComponent)
	{
		return AbilitySystemComponent->TryActivateAbilityByClass(*FoundAbility, bAllowRemoteActivation);
	}
...
void ARPGCharacterBase::Restart()
{
	Super::Restart();
	...
	if (AbilitySystemComponent)
	{
		ARPGCharacterBase* CharacterBase = Cast<ARPGCharacterBase>(this);
		if (CharacterBase)
		{
